...
export default function TenantOverviewPage() {
    ...
    return (
        <div className="md:p-8  min-h-screen">
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-3xl p-6 md:p-8 mb-8 shadow-lg">
                <h1 className="text-2xl md:text-3xl font-bold mb-2">Hello, {session?.user?.name?.split(' ')[0]} 👋</h1>
                <p className="text-blue-100 opacity-90 text-sm md:text-base">Welcome back to your dashboard. Here is your latest property activity.</p>
            </div>
                ...
                {stats?.recentActivities?.length > 0 ? (
                    <div className="space-y-4">
                        {stats.recentActivities.map((act, index) => (
                            <div key={index} className="flex flex-col sm:flex-row sm:items-center p-4 rounded-2xl bg-zinc-50 hover:bg-zinc-100 transition-colors gap-3 sm:gap-4">
                                <div className="hidden sm:block w-2 h-2 bg-blue-500 rounded-full"></div>
                                <div className="flex-1">
                                    <p className="font-semibold text-zinc-800 text-sm sm:text-base">Booked {act.propertyTitle}</p>
                                    <p className="text-xs text-zinc-400">{new Date(act.createdAt).toLocaleDateString()}</p>
                                </div>
                                <span className="w-fit text-[10px] sm:text-xs font-medium px-3 py-1 bg-white border border-zinc-200 rounded-full text-zinc-600">
                                    Pending
                                </span>
                            </div>
                        ))}
